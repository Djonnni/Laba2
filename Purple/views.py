...
class PlanesView(View):
    ...
    def post(self, *args, **kwargs):
         ...
         Plane.objects.create(airline = airline_value, privileges = privileges_value, type_of_plane = type_of_plane_value, place_count = place_count_value )
         return  redirect('/')
...
###
    def post(self, *args, **kwargs):
         ...
         Trip.objects.create(point_of_departure = point_of_departure_value, point_of_destination = point_of_destination_value,
         data_of_departure = data_of_departure_value, time_of_departure = time_of_departure_value,
         data_of_destination = data_of_destination_value, time_of_destination = time_of_destination_value,
         plane_number = Plane.objects.filter(id=int(plane_number_value))[0], cost = cost_value)
         return  redirect('/')
...
###
    def post(self, *args, **kwargs):
         ...
         Tickets.objects.create(trip_number = Trip.objects.filter(id=int(trip_number_value))[0],
         passenger_id = Passport_data.objects.filter(id=int(passenger_id_value))[0], ticket_id = ticket_id_value,
         purchase_data = purchase_data_value, purchase_time = purchase_time_value, purchase_cost = purchase_cost_value)
         return  redirect('/')
